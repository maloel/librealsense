...
// 
// NOTE: without _RSCORE_EXPAND, the result is _RSCORE_MODULE_NAME_registry_entry
//
#define _RSCORE_EXPAND( X ) X
#define REGISTER_RSCORE_MODULE( module, name )                                                                         \
    extern "C" size_t _RSCORE_EXPAND( _RSCORE_MODULE_NAME )##_registry_entry = module_registry::add< module >( name )
...
class module_registry
{
    ...
    //
    using factory_fn =  //
        std::function< std::shared_ptr< context_module >( std::string const & name, context const & ) >;
...
public:
    ...
    static size_t add( std::string const & name )
    {
        ...
        auto module_was_inserted
            = registry
                  .emplace( name,
                            []( std::string const & name ) -> std::shared_ptr< context_module >
                            {
                                std::shared_ptr< T > ctx_module;
                                try
                                {
                                    ctx_module = std::make_shared< T >( name );
                                }
    ...
    //
    static std::vector< std::shared_ptr< context_module > > create_context_modules( context const & );
};
