...
class rsbackend_module : public context_module
{
...
public:
    rsbackend_module( std::string const & name, context & ctx )
        : super( name, ctx )
    {
    }
...
public:
    ...
    {
        // Missing "backend" in settings: enabled; use defaults
        // If "backend" is not an object: disabled
    ...
    std::shared_ptr< device_factory > create_device_factory( device_factory::callback && cb ) override
    {
        ...
        return std::make_shared< backend_device_factory >( _context, std::move( cb ) );
    }
};


REGISTER_RSCORE_MODULE( rsbackend_module, "backend" );
