...
void dds_device::wait_until_online( size_t timeout_ms )
{
    ...
    {
        if( timer.has_expired() )
            DDS_THROW( runtime_error, "timeout waiting for '" << debug_name() << "' to come online" );
        std::this_thread::sleep_for( std::chrono::milliseconds( 500 ) );
    }
    while( ! is_online() );
}
...
void dds_device::on_discovery_restored( topics::device_info const & new_info )
{
    ...
    if( new_info.to_json().nested( "product-line" ).string_ref_or_empty()
        != device_info().to_json().nested( "product-line" ).string_ref_or_empty() )
        DDS_THROW( runtime_error, "device product line cannot change" );
