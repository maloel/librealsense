...
bool type_from_value( json const & j, std::string & type )
{
    ...
    case json::value_t::number_float:
        if( type.length() == 5 && type == "float" )
            return true;
        if( type.empty() || type == "integer" || type == "unsigned" )
    ...
    case json::value_t::number_integer:
        if( type.length() == 7 && type == "integer" )
            return true;
        if( type.empty() )
            return type.assign( "integer", 7 ), true;
...
bool type_from_value( std::string & type, json const & j )
{
    ...
    case json::value_t::number_unsigned:
        ...
        if( type.empty() || type == "integer" )
            return type.assign( "unsigned", 8 ), true;  // unsigned trumps integer
        if( type == "float" )
            return true;  // float trumps unsigned
        break;

    case json::value_t::number_integer:
        // Numbers are signed by default: only if 63-bits aren't enough will it go to 'unsigned'.
        // E.g.,
        //      12345678901234567890 is unsigned
        //      1234567890123456789  is signed
        if( type.length() == 7 && type == "integer" )
            return true;
        if( type.empty() )
            return type.assign( "integer", 7 ), true;
        else if( type == "float" || type == "unsigned" )
            return true;  // float/unsigned trumps integer
        break;
