    ...
    : super( ctx )
{
        ...
        || dds_settings.is_object() && dds_settings.nested( std::string( "enabled", 7 ) ).default_value( true ) )
    {
        ...
        if( ! _participant->is_valid() )
        {
            ...
            // 
            // FastDDS does provide a 'udp/max-message-size' setting, but this applies to both send and receive. We want
            // to only limit sends! So we use a new property of FastDDS:
